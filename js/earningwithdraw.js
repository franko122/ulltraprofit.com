...
withdrawBtn.addEventListener("click",async()=>{
    if(!loaded){
        ...
        if(amountValue<=0){
        ...
        else if(amountValue>userDetails.earnings){
            alert("Insufficient funds!")
        }
        else{
            
            // withdraw the funds from user
            const newEarnings=userDetails.earnings-Number(amountValue)
            ...
            await fetch(`${apiEntry}/transactions/withdraw`,{
                ...
                body:JSON.stringify({amount:amountValue,wallet_id:userDetails.wallet_id,source:"earnings"})
            }).then(res=>res.json()).then(data=>{
                console.log(data)
                if(data.success){
            ...
            fetch(`${apiEntry}/users/update/${userDetails._id}`,{
                ...
                body:JSON.stringify({earnings:newEarnings})
            }).then(res=>res.json()).then((data)=>{
                if(data.success){
