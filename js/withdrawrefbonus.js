...
withdrawBtn.addEventListener("click",async()=>{
    if(!loaded){
        ...
        if(amountValue<=0){
        ...
        else if(amountValue>userDetails.referralBonus){
            alert("Insufficient funds!")
        }
        else{
            
            // document the transaction
            const newReferralBonus=userDetails.referralBonus-Number(amountValue)
            ...
            await fetch(`${apiEntry}/transactions/withdraw`,{
                ...
                body:JSON.stringify({amount:amountValue,wallet_id:userDetails.wallet_id,source:"referral bonus"})
            }).then(res=>res.json()).then(data=>{
                console.log(data)
                if(data.success){
            ...
            fetch(`${apiEntry}/users/update/${userDetails._id}`,{
                ...
                body:JSON.stringify({referralBonus:newReferralBonus})
            }).then(res=>res.json()).then((data)=>{
                if(data.success){
